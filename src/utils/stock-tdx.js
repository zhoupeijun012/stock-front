...
export class TrendIndicators {
   ...
   */
  static result(data) {
    const HHJSJDA = data.map((item) =>
      this.calculateHHJSJDA(item.close, item.open, item.low, item.high)
    );

    const HHJSJDB = data.map((_, index) =>
      this.calculateHHJSJDB(HHJSJDA, index)
    );
...
export class MajorIndicator {
  ...
  static result(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const XYS0 = refWY1004 !== 0 
        ? ((WY1004[i] - refWY1004) / refWY1004) * 100 
        : 0; // 防止除以0
      ...
      const CYS13 = SSRCBJX !== 0 
        ? ((close - SSRCBJX) / SSRCBJX) * 100 
        : 0;
...
export class FlowIndicator {
  ...
  static result(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // 计算资金运转和资金流失 (STICKLINE对应的值)
      // const 资金运转 = VAR1 > VAR2 ? VAR1 : 0;
      // const 资金流失 = VAR1 < VAR2 ? VAR1 : 0;
      ...
      result.push({
        ...
        // 资金流失,
        // 颜色标记（用于可视化）
        // color: VAR1 > VAR2 ? "red" : VAR1 < VAR2 ? "green" : "gray",
      });
    }
  ...
  static calculateSMA(values, period, weight) {
    ...
    for (let i = 0; i < values.length; i++) {
      if (i === 0) {
        ...
        result.push(
          (weight * values[i] + (period - weight) * result[i - 1]) / period
        );
      }
    }
